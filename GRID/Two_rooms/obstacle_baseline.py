    ...
    for num_agent in [5, 10]:
        ...
        for iter in range(max_iteration):
            for index in range(num_agent):
                ...
                Q_learning_i[index].set_Q_value(Q)
            all_steps = 0
            for i in range(max_episode):
                ...
                while not done:
                    ...
                    for index in range(num_agent):
                        ...
                        action = Q_learning_i[index].choose_action(s, epsilon)
                    ...
                    for index in range(num_agent):
                        ...
                        rewards[index] += r
                        Q_learning_i[index].learn(s,a,r,s_)
                    state = next_state
                rewards_agent[int(num_agent / 5 - 1)][iter][i] = np.mean(rewards)
                print(i, count)
                all_steps += count
            print(iter)
    np.save("baseline_rewards.npy", rewards_agent)
