...
if __name__ == '__main__':
    ...
    for num_agent in [5, 10]:
        ...
        VBAE = [WToE_Q_learning(args, map_size*map_size, 5) for _ in range(num_agent)]
        Repaly_buffer = [WToE_Buffer(args) for _ in range(num_agent)]
        visited_num = np.zeros([max_episode, map_size, map_size])
            ...
            for i in range(max_episode):
                ...
                while not done:
                    ...
                    for index in range(num_agent):
                        ...
                        visited_num[i][state[index][0]][state[index][1]] = visited_num[i][state[index][0]][state[index][1]] + 1
                        if count == 0:
                        ...
                        action, flag, z_norm = VBAE[index].choose_action(s, epsilon, count, state_stack[index], action_stack[index], reward_stack[index])
