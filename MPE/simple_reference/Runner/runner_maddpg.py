...
class Runner:
    ...
    def run(self, runner_time):
        ...
        for time_step in tqdm(range(self.args.time_steps)):
            ...
            if time_step % self.episode_limit == 0:
                s = self.env.reset()
                s, r, done, info = self.env.step([np.array([0, np.random.rand() * 2 - 1, 0, np.random.rand() * 2 - 1, 0]), np.array([0, np.random.rand() * 2 - 1, 0, np.random.rand() * 2 - 1, 0])])
            u = []
            actions = []
            with torch.no_grad():
    ...
    def evaluate(self):
        ...
        for episode in range(self.args.evaluate_episodes):
            # reset the environment
            s = self.env.reset()
            s, r, done, info = self.env.step([np.array([0, np.random.rand() * 2 - 1, 0, np.random.rand() * 2 - 1, 0]),
                                              np.array([0, np.random.rand() * 2 - 1, 0, np.random.rand() * 2 - 1, 0])])
            rewards = 0
