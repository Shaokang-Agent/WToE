...
class Runner:
    ...
    def _init_agents(self):
        ...
        for i in range(self.args.n_agents):
            agent = NoisyNet_Agent(i, self.args)
            agents.append(agent)
        return agents
            ...
            with torch.no_grad():
                ...
                for i in range(self.args.n_agents, self.args.n_players):
                    ...
                    if time_step == 0:
                        ...
                        s_stack[agent_id].append(s[i])
                    action = self.adv_agents[agent_id].select_action(s[i], self.noise, self.epsilon, time_step, copy.deepcopy(s_stack[agent_id]), copy.deepcopy(a_stack[agent_id]), copy.deepcopy(r_stack[agent_id]))
                    if time_step == 0:
                        for index in range(self.args.history_size+2):
