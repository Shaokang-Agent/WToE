...
class PR2:
    ...
    def train(self, transitions, other_agents):
        ...
        for agent_id in range(self.args.n_agents):
            o[agent_id] = torch.from_numpy(np.array(o[agent_id])).float()
            o_next[agent_id] = torch.from_numpy(np.array(o_next[agent_id])).float()
        s = torch.cat(o, dim=1)
        s_next = torch.cat(o_next, dim=1)
        if self.args.cuda:
            r = r.cuda()
            u = u.cuda()
            s = s.cuda()
            s_next = s_next.cuda()
            for agent_id in range(self.args.n_agents):
                o[agent_id] = o[agent_id].cuda()
                o_next[agent_id] = o_next[agent_id].cuda()
        ...
        if self.args.level_k > 1:
            ...
            opponent_actions = all_action[-2].reshape(-1, self.args.batch_size, 4)
        ...
        if self.args.level_k > 3:
            actions = all_action[-3].unsqueeze(dim=0)
            opponent_actions = all_action[-4].reshape(-1, self.args.batch_size, 4)
            u_k = torch.cat((actions, opponent_actions), dim=0)
            if self.args.cuda:
                u_k = u_k.cuda()
            q_k = self.critic_network(s, u_k)
        ...
        u_2 = copy.deepcopy(u)
        opponent_actions = self.opponent(o[self.agent_id], u[self.agent_id]).reshape(-1, self.args.batch_size, 4)
        index = 0
